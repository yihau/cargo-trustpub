...
fn lists_github_configs() {
    ...
    let mock = server.mock(|when, then| {
        when.method(GET)
            ...
            .query_param("crate", "cargo-trustpub")
    ...
    let configs = client(&server)
        .list_github_configs("cargo-trustpub")
        .unwrap();
...
fn omits_environment_when_absent() {
    ...
    client(&server)
        .add_github_config("cargo-trustpub", "yihau", "cargo-trustpub", "ci.yml", None)
        .unwrap();
...
fn lists_gitlab_configs() {
    ...
    let configs = client(&server)
        .list_gitlab_configs("cargo-trustpub")
        .unwrap();
...
fn reads_trustpub_only() {
    ...
    let only = client(&server).trustpub_only("cargo-trustpub").unwrap();
    mock.assert();
    assert!(only);
}
...
fn sets_trustpub_only() {
    ...
    let mock = server.mock(|when, then| {
        when.method(httpmock::Method::PATCH)
            .path("/api/v1/crates/cargo-trustpub")
            .header("authorization", TOKEN)
            .json_body(json!({ "crate": { "trustpub_only": true } }));
        then.status(200)
            .json_body(json!({ "crate": { "trustpub_only": true } }));
    ...
    client(&server)
        .set_trustpub_only("cargo-trustpub", true)
        .unwrap();
    mock.assert();
}
